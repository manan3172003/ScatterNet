...
export default function Post({ post, onPostClick, onCommentClick }) {
  ...
  async function handleLike() {
    ...
    const response = await fetch(`${post.author.id}/inbox`, {
      ...
      body: JSON.stringify({
        ...
        id: `http://localhost:8000/api/authors/${
          user.author_id
        }/liked/${crypto.randomUUID()}`,
        object: `${post.id}`,
      }),
    });
    if (!response.ok) {
  ...
  async function getLikeStatus(user) {
    const response = await fetch(
      `http://localhost:8000/api/authors/${user.author_id}/liked`
    );
    if (response.ok) {
      ...
      <div className="post-body">
        <div className="post-icons">
          ...
          {post.visibility === "PUBLIC" ? (
            <Share2 size={24} onClick={handleShare} />
          ) : (
            <></>
          )}
        </div>
        {post.visibility === "PUBLIC" || post.visibility === "UNLISTED" ? (
          <div className="likes">{post.likes.count} likes</div>
        ) : (
          <></>
        )}
